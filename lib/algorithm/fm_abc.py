...
class FixMatchABC(FixMatch):
    ...
    def __init__(self, cfg: CfgNode) -> None:
        ...
        self.qhat_reg = (torch.ones([1, self.num_classes], dtype=torch.float) / self.num_classes).cuda()
        self.qhat_cb = (torch.ones([1, self.num_classes], dtype=torch.float) / self.num_classes).cuda()
        self.qhat_m = 0.999
        self.tau = 0.5
    ...
    def run_step(self) -> None:
        ...
        mean_prob = p_abc.detach().mean(dim=0)
        self.qhat_cb = self.qhat_m * self.qhat_cb + (1 - self.qhat_m) * mean_prob

        delta_logits = torch.log(self.qhat_cb)
        logits_strong_abc = logits_strong_abc + self.tau * delta_logits

        with torch.no_grad():
            conf_abc, pred_class_abc = torch.max(p_abc, dim=1)
            energy = -torch.logsumexp(logits_weak_abc, dim=1)

            #loss_weight_abc = conf_abc.ge(self.conf_thres).float()
            loss_weight_abc = energy.le(self.conf_thres).float()
        ...
        mean_prob = p.detach().mean(dim=0)
        self.qhat_reg = self.qhat_m * self.qhat_reg + (1 - self.qhat_m) * mean_prob

        with torch.no_grad():
            confidence, pred_class = torch.max(p, dim=1)
            loss_weight = confidence.ge(self.conf_thres).float()

        # AML for vanilla classiifer
        delta_logits = torch.log(self.qhat_reg)
        logits_strong = logits_strong + self.tau * delta_logits
